...
def test():
    # TEST WITHOUT get_json kwargs
    with app.test_request_context(method='POST', headers={'Authorization': 'Bearer test_token', 'Content-Type': 'application/json'}, data=json.dumps({"uid": "test_uid"})):
        req = request
        ...
        with patch('admin.auth.verify_id_token') as mock_verify:
            ...
            with patch('admin.firestore.client') as mock_client:
                ...
                mock_target_doc.to_dict.return_value = {'role': 'cartorio_admin', 'cartorio_id': 'some_cartorio'}
