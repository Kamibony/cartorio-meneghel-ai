...
class DocumentValidator:
    ...
    def validate(self) -> List[Dict[str, str]]:
        ...
        # CPF Check
        expected_cpf = self.ground_truth.get("cpf")
        if expected_cpf:
            norm_expected_cpf = normalize_digits(expected_cpf)
        ...
        # RG Check
        expected_rg = self.ground_truth.get("rg")
        if expected_rg:
            norm_expected_rg = normalize_digits(expected_rg)
        ...
        # Filiation Check
        expected_mae = self.ground_truth.get("nome_mae")
        if expected_mae:
            norm_expected_mae = normalize_string(expected_mae)
            norm_typed_text = normalize_string(self.typed_text)

            if norm_expected_mae not in norm_typed_text:
                self.errors.append({
                    "field": "nome_mae",
                    "level": "critical",
                    "message": f"Expected '{norm_expected_mae}' as substring but not found in text"
                })
